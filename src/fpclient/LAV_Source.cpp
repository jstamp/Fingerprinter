...
void LAV_Source::init(const string& fileName)
{
    // Assume that we want to start fresh
    if ( d->inFormatContext || d->inCodecContext )
        release();

    if ( avformat_open_input(&d->inFormatContext, fileName.c_str(), NULL, NULL ) < 0 )
    {
...
void LAV_Source::getInfo(const string& fileName, int& lengthSecs, int& samplerate, int& bitrate, int& nchannels )
{
